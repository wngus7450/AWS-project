...
여행 계획을 관리하고 AI(Gemini, Nova)로 여행지 추천과 일정 조언을 받는 풀스택 웹 애플리케이션
...
- **Trip CRUD**: 여행 계획 생성(출발일/도착일 설정), 조회(최신순), 개별 삭제, 전체 삭제
...
앱 실행 후 아래 내용으로 여행 계획을 등록하여 테스트할 수 있습니다:
...
| 출발일 | 2026-05-01 |
| 도착일 | 2026-05-04 |
| 여행 계획 상세 | 성산일출봉, 우도, 협재해수욕장 방문 예정. 흑돼지 맛집 탐방. |

### 테스트 순서

1. 위 샘플 데이터로 여행 계획 등록
2. 등록된 카드에서 "Gemini 추천" 또는 "Nova 조언" 버튼 클릭
3. AI 응답이 카드에 표시되는지 확인
4. 카드의 무결성 배지(✅ 검증됨)가 정상 표시되는지 확인
5. 개별 삭제 및 전체 삭제 기능 확인
